...
const defaultStory = `Pros Immobilier est né d’un constat simple : les agences perdent encore trop de temps entre les cahiers, les feuilles de calcul et des outils qui ne communiquent pas entre eux. Nous avons créé la plateforme qui rassemble enfin tout leur travail.

Notre ambition est de rendre la gestion locative plus fluide, plus fiable et plus transparente. Chaque fonctionnalité répond à une réalité du terrain, du suivi d’un loyer au reversement d’un propriétaire, en passant par la maintenance.

Nous avançons aux côtés des professionnels de l’immobilier pour bâtir un outil utile aujourd’hui et capable d’accompagner leur croissance demain.`;

export default function About({ websiteContent = {} }) {
    const [openFaq, setOpenFaq] = useState(0);
    const story = websiteContent.story || defaultStory;
    const commitments = websiteContent.commitments?.length ? websiteContent.commitments : defaultCommitments;
    const faqs = websiteContent.faqs?.length ? websiteContent.faqs : defaultFaqs;
                    ...
                    <div className="mx-auto grid max-w-5xl gap-12 lg:grid-cols-[0.8fr_1.2fr] lg:gap-24">
                        <div>
                            ...
                            <h2 className="mt-5 text-4xl font-medium leading-tight tracking-[-0.04em] text-[#0b1730] sm:text-5xl">
                                {websiteContent.missionTitle || 'Simplifier pour mieux gérer.'}
                            </h2>
                        </div>

                        <div>
                            <RichContent
                                content={websiteContent.missionText || 'Donner aux agences une vision claire de leur activité et les libérer des tâches répétitives, afin qu’elles puissent se concentrer sur leurs clients et leur développement.'}
                                className="text-xl leading-8 text-[#0b1730] sm:text-2xl sm:leading-10"
                            />
                            ...
                            {faqs.map((faq, index) => {
                                ...
                                return (
                                    ...
                                    >
                                        ...
                                        {isOpen ? (
                                            <RichContent content={faq.answer} className="max-w-2xl pb-6 pr-10 text-sm leading-7 text-slate-600 sm:text-base" />
                                        ) : null}
                                    </div>
                                );
                            })}
