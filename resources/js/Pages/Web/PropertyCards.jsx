    ...
    return (
        <div className="grid gap-6 md:grid-cols-2 lg:grid-cols-3">
            {properties.map((property, index) => (
                <Link key={property.id} href={`/biens/${property.id}`} className="group overflow-hidden rounded-3xl bg-white shadow-[0_14px_45px_rgba(17,31,61,0.08)] transition hover:-translate-y-1" aria-label={`Voir le détail de ${property.title}`}>
                    ...
                    <div className="p-6">
                        <p className="text-xs font-bold uppercase tracking-[0.16em] text-[#76c206]">{property.reference || 'Bien disponible'}</p>
                        <h3 className="mt-2 truncate text-xl font-bold text-[#111f3d]">{property.title}</h3>
