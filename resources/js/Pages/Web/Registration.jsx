                    ...
                    <form onSubmit={submit} className="border-t border-[#0b1730]/15 pt-8">
                        ...
                        <FormSection icon={Building2} title="Informations de l’agence">
                            <div className="grid gap-x-5 gap-y-6 sm:grid-cols-2">
                                <Field label="Nom de l’agence" error={form.errors.name}><Input value={form.data.name} onChange={(e) => form.setData('name', e.target.value)} placeholder="Ex. Agence Horizon" /></Field>
                                <Field label="Téléphone" error={form.errors.tel1}><PhoneInput value={form.data.tel1} onChange={(value) => form.setData('tel1', value)} placeholder="07 00 00 00 00" /></Field>
                                <Field label="Email de l’agence" error={form.errors.email1}><Input type="email" value={form.data.email1} onChange={(e) => form.setData('email1', e.target.value)} placeholder="contact@agence.ci" /></Field>
                                <Field label="Adresse" error={form.errors.adresse}><Input value={form.data.adresse} onChange={(e) => form.setData('adresse', e.target.value)} placeholder="Cocody, Abidjan" /></Field>
                                <Field label="Région" error={form.errors.region}>
                                    <Select value={String(form.data.region)} onValueChange={(value) => { form.setData('region', value); form.setData('ville_id', ''); }}>
                                        <SelectTrigger>
                                ...
                                <Field label="Ville" error={form.errors.ville_id}>
                                    <Select value={String(form.data.ville_id)} onValueChange={(value) => form.setData('ville_id', value)} disabled={!form.data.region}>
                                        <SelectTrigger disabled={!form.data.region}>
                        ...
                        <FormSection icon={UserRound} title="Compte du responsable" subtitle="Les identifiants utilisés pour vous connecter.">
                            <div className="grid gap-x-5 gap-y-6 sm:grid-cols-2">
                                <Field label="Nom complet" error={form.errors.new_responsable_name}><Input value={form.data.new_responsable_name} onChange={(e) => form.setData('new_responsable_name', e.target.value)} placeholder="Nom et prénoms" /></Field>
                                <Field label="Téléphone personnel" error={form.errors.new_responsable_tel1}><PhoneInput value={form.data.new_responsable_tel1} onChange={(value) => form.setData('new_responsable_tel1', value)} placeholder="07 00 00 00 00" /></Field>
                                <Field label="Email de connexion" error={form.errors.new_responsable_email} className="sm:col-span-2"><Input type="email" value={form.data.new_responsable_email} onChange={(e) => form.setData('new_responsable_email', e.target.value)} placeholder="responsable@agence.ci" /></Field>
                                <Field label="Mot de passe" error={form.errors.new_responsable_password}><PasswordInput value={form.data.new_responsable_password} onChange={(e) => form.setData('new_responsable_password', e.target.value)} placeholder="8 caractères minimum" /></Field>
                                <Field label="Confirmation" error={form.errors.new_responsable_password_confirmation}><PasswordInput value={form.data.new_responsable_password_confirmation} onChange={(e) => form.setData('new_responsable_password_confirmation', e.target.value)} placeholder="Confirmer le mot de passe" /></Field>
                            </div>
                        </FormSection>

                        <label className="mt-7 flex cursor-pointer items-start gap-3 text-xs leading-5 text-slate-500">
                            <input type="checkbox" checked={form.data.accept_terms} onChange={(e) => form.setData('accept_terms', e.target.checked)} className="mt-0.5 h-4 w-4 shrink-0 rounded accent-[#76c206]" />
...
function Field({ label, error, className = '', children }) {
    return <div className={`space-y-1.5 ${className}`}><label className="block text-sm font-medium text-[#0f172a]">{label}</label>{children}{error ? <p className="text-xs text-[#b42318]">{error}</p> : null}</div>;
}
