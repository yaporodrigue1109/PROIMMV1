...
@section('content')
    <div class="page">
        ...
        <div class="page-header">
            <div class="page-header-copy">
                ...
                <p>Renseignez les informations de la propriété, ses bâtiments et ses portes.</p>
            </div>
        </div>

        {{-- Toast de progression --}}
        <div id="toast-progress" class="toast-progress" style="display: none;">
            <div class="toast-content">
                <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor">
                    <path stroke-linecap="round" stroke-linejoin="round" d="M9 12.75 11.25 15 15 9.75M21 12a9 9 0 1 1-18 0 9 9 0 0 1 18 0Z" />
                </svg>
                <span>Propriété enregistrée avec succès !</span>
            </div>
        </div>
        ...
        <form action="{{ route('agence.proprietes.store') }}" method="POST" id="form-propriete">
            @csrf
                        ...
                        <div class="form-grid form-grid-3">
                            ...
                            <div class="form-field form-field-wide">
                                ...
                                <select name="proprietaire_id" id="proprietaire_id" required
                                    ...
                                    @foreach($proprietaires as $p)
                                        <option value="{{ $p->proprietaire_id }}" @selected(old('proprietaire_id') == $p->proprietaire_id)>
                                            {{ $p->name }}
                                        </option>
                                    @endforeach
                                </select>
                            ...
                            <div class="form-field ">
                                ...
                                <select name="lot_id" id="lot_id">
                                    ...
                                    @foreach($lots as $lot)
                                        <option value="{{ $lot->lot_id }}" @selected(old('lot_id') == $lot->lot_id)>
                                            {{ $lot->libelle }} – {{ $lot->commune }}
                                        </option>
                                    @endforeach
                                </select>
                                @error('lot_id')
                            ...
                            <div class="form-field form-field-wide">
                                <label for="adresse_complete">Video Url</label>
                            ...
                            <div class="form-field form-field-wide">
                                ...
                                <textarea name="description" id="description" rows="3"
                                          placeholder="Décrivez la propriété (superficie, environnement, particularités…)">{{ old('description') }}</textarea>
                                @error('description')
                                <span class="form-error">{{ $message }}</span>
                                @enderror
                            </div>
                            ...
                            <div class="form-field">
                                <label class="checkbox-label">
                                    ...
                                    <input type="checkbox" name="is_allocation" value="1" @checked(old('is_allocation')) />
            ...
            <div class="create-section" data-section="2">
                <div class="create-section-aside">
                    ...
                    <div>
                        ...
                        <p class="create-section-hint">Ajoutez un ou plusieurs bâtiments, chacun avec ses unités locatives et tarifs.</p>
                    </div>
                </div>

                <div class="create-section-body">
                    <div id="batiments-container">
                        <div class="batiment-block" data-index="0">
            ...
            <div class="form-actions">
                <a href="{{ route('agence.proprietes.index') }}" class="btn btn-outline">
                    <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor">
    ...
    @push('styles')
        <style>
            ...
            .batiment-meta .form-field { flex: 1; margin: 0; }
            .batiment-meta .form-field label { font-size: .7rem; }
            ...
            /* ═══════════════════════════════════════════
               Summary + Actions + Alert + Toast
            ═══════════════════════════════════════════ */
            .summary-section {
                background: linear-gradient(135deg, var(--color-success-light) 0%, #e0f2fe 100%);
                border-radius: var(--radius-lg);
                padding: 1rem;
                margin-bottom: 2rem;
                border: 1px solid var(--color-success);
            }
            .summary-header { display: flex; align-items: center; gap: .5rem; margin-bottom: .75rem; }
            .summary-header svg { width: 1.2rem; height: 1.2rem; color: var(--color-success); }
            .summary-header h4 { margin: 0; font-size: .9rem; font-weight: 600; color: var(--color-text); }
            #summary-content { font-size: .85rem; color: var(--color-text); line-height: 1.6; }
            ...
            .btn-secondary { background: var(--color-bg-subtle); border: 1px solid var(--color-border-dark); color: var(--color-text); }
            ...
            .toast-progress { position: fixed; top: 1rem; right: 1rem; background: var(--color-success); color: #fff; padding: .75rem 1rem; border-radius: var(--radius-md); box-shadow: var(--shadow-lg); z-index: 1000; animation: slideIn .3s ease; }
            .toast-content { display: flex; align-items: center; gap: .5rem; }
            .toast-content svg { width: 1rem; height: 1rem; }
            ...
            .proximites-header svg {
                width: .95rem;
                height: .95rem;
                color: var(--color-primary);
                flex-shrink: 0;
            }
            .proximites-header .prox-title {
                font-size: .75rem;
                font-weight: 700;
                text-transform: uppercase;
                letter-spacing: .05em;
        ...
        <script>
            (function () {
                ...
                document.getElementById('btn-preview').addEventListener('click', () => {
                    const proprietaire = document.getElementById('proprietaire_id').selectedOptions[0]?.text || 'Non renseigné';
                    const type         = document.getElementById('type_propriete_id').selectedOptions[0]?.text || 'Non renseigné';
                    const nbBatiments  = document.querySelectorAll('.batiment-block').length;
                    const nbPortes     = document.querySelectorAll('.porte-block').length;
                    const allocation   = document.querySelector('[name="is_allocation"]:checked')?.value === '1' ? 'Oui' : 'Non';

                    document.getElementById('summary-content').innerHTML = `
                <strong>Propriétaire :</strong> ${proprietaire}<br>
                <strong>Type :</strong> ${type}<br>
                <strong>Bâtiments :</strong> ${nbBatiments}<br>
                <strong>Portes :</strong> ${nbPortes}<br>
                <strong>Allocation :</strong> ${allocation}
            `;
                ...
                // ─── Délégation — bâtiments container ─────────────────
